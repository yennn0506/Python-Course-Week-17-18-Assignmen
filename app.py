...
if "initialized" not in st.session_state:
    if os.path.exists(HISTORY_DIR):
        shutil.rmtree(HISTORY_DIR)  # 清空舊的失敗照片
...
if not os.path.exists(FONT_FILENAME):
    try:
        light_font_url = "https://github.com/fondfont/noto-sans-tc-subsets/raw/master/fonts/NotoSansTC-Regular.ttf"
        urllib.request.urlretrieve(light_font_url, FONT_FILENAME)
...
.canvas-title, .canvas-title * {
    ...
    font-family: "Songti TC", "Songti SC", "STSong", "BiauKai", serif !important;
...
def generate_polaroid(image_file, text, date_str):
    ...
    cloud_fonts = [
        ...
        "/System/Library/Fonts/STSongti-Light.ttc", 
    ]
    
    for font_path in cloud_fonts:
        if os.path.exists(font_path):
            try:
                font1 = ImageFont.truetype(font_path, 42) # 稍微放大讓英文字更顯眼漂亮
    ...
    # 🌟 萬一環境抓不到外部字型，直接調用 Pillow 內建字型（因為是純英文，必能成功顯影！）
    if not font_loaded:
        try:
            font1 = ImageFont.load_default(size=42)
            font2 = ImageFont.load_default(size=26)
        except:
            font1 = ImageFont.load_default()
            font2 = ImageFont.load_default()

    # ✍️ 印出純英文心情與時間（座標微調確保視覺平衡）
    draw.text((45, 795), text, fill="#4A4A4A", font=font1)
    draw.text((45, 875), date_str, fill="#8C7365", font=font2)
...
if st.session_state.page == "home":
    ...
    <div class="polaroid-card" style="text-align: center;">
        ...
        <p style="color: #8C7365; font-size: 14px; margin-bottom: 25px;">把散落的日常碎片，裁切成一頁頁的拍立得回憶。</p>
    </div>
    """, unsafe_allow_html=True)
...
# =========================
    ...
    if file:
        ...
        with c1:
            # 🌟【核心更改】將原本的文字輸入框改成安全又好看的英文心情下拉選單
            mood_options = ["Busy", "Relaxing", "Tiring", "Happy", "Boring", "Productive"]
            text = st.selectbox("今日心情狀態 (Mood)", options=mood_options)
...
# =========================
    ...
    if files:
        ...
        st.markdown('<p style="color: #8C7365;">目前牆上空空如也，快去 上傳回憶 留下一點足跡吧！</p>', unsafe_allow_html=True)
